...
The `version` declaration was introduced in `Rampart/2.10`
...
The `metadata` declaration was introduced in `Rampart/2.10`
...
As described in more detail in the **Rampart Mod** section, it is possible to declare a `metadata()` statement since `Rampart/2.10`, which can be declared either at the mod or rule levels. This section describes the use case at the rule level.
...
The API Protect directives `api()` and `input()`, introduced in Rampart 2.10, provide an additional protection mechanism for API requests on top of the existing protection that the supported rule otherwise provides. For example, for the Filesystem rule, the `api()` and `input()` directives can be specified in addition to the primary read() and write() selectors for this rule. It is therefore possible to specify multiple Rampart rules which are almost identical except for the presence of the API Protect directives.
...
Support for API Protect was added to this rule in Rampart 2.10. Please see the **API Protect Directives** page in the Rampart documentation for information on how to configure this rule for API endpoint protection.
